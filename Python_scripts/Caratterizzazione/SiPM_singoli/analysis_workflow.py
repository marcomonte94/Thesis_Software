...
def wf_data(wf_path, res_path, n_wf, threshold):
    ...
    for input_file in allMyData[:n_wf]:
        ...
        peaks, _ = find_peaks(amplDLED, height=threshold, prominence=0.008)
        ...
        for i in range(2, len(peaks)-2):
            ...
            if peak_timestamp[i] - peak_timestamp[i-1] > 500e-9 and peak_timestamp[i+1] - peak_timestamp[i] > 500e-9:
...
def gain(areas):
    ...
    plt.figure()
    y_data, edges, _ = plt.hist(areas, bins=150)
    x_data = 0.5 * (edges[1:] + edges[:-1])

    peakHisto, _ = find_peaks(y_data, height=500, prominence=200)
...
def make_scatterplot(all_delay, all_amplitude):
    ...
    plt.figure()
    bins_x = 10 ** np.linspace(-8.5, -4, 150)
...
if __name__ == '__main__':
    ...
    path = 'C:/Users/Marco/Desktop/id5'
    res_path = 'C:/Users/Marco/Desktop/Analisi_SiPM/Caratterizzazione/id5' 
    areas, all_amplitude, all_delay = wf_data(path, res_path, 50, 0.015)
